...
const mastery = () => ({ ...database.prepare(`SELECT successes, required_successes AS requiredSuccesses,
  misses, artist_misses AS artistMisses, title_misses AS titleMisses, active FROM mistake_mastery WHERE track_key = 'artist—song'`).get() });

applyMistakeResult(database, result(0, 1));
assert.deepEqual(mastery(), { successes: 0, requiredSuccesses: 2, misses: 1, artistMisses: 1, titleMisses: 0, active: 1 });
applyMistakeResult(database, result(1, 1));
assert.deepEqual(mastery(), { successes: 1, requiredSuccesses: 2, misses: 1, artistMisses: 1, titleMisses: 0, active: 1 });
applyMistakeResult(database, result(1, 0));
assert.deepEqual(mastery(), { successes: 0, requiredSuccesses: 2, misses: 2, artistMisses: 1, titleMisses: 1, active: 1 });
applyMistakeResult(database, result(1, 1));
applyMistakeResult(database, result(1, 1));
assert.deepEqual(mastery(), { successes: 2, requiredSuccesses: 2, misses: 2, artistMisses: 1, titleMisses: 1, active: 0 });
applyMistakeResult(database, result(1, 1));
assert.deepEqual(mastery(), { successes: 2, requiredSuccesses: 2, misses: 2, artistMisses: 1, titleMisses: 1, active: 0 });
...
assert.deepEqual(mastery(), { successes: 1, requiredSuccesses: 2, misses: 1, artistMisses: 1, titleMisses: 1, active: 1 });
