...
import { Play, Pause, Film, LockKeyhole, Check, Share2, ArrowLeft, Users, Trophy, Clock3, Send, X, RotateCcw } from "lucide-react";
import { chanceAt, chanceClock, questionMaximum, roundAt, nextAnswerAt, historyPlayback, type VideoEpisode, type VideoAttempt, type VideoDraft, type VideoHistory, type VideoReport } from "../shared/video-quiz";
...
export default function VideoQuiz({ shareToken }: { shareToken?: string }) {
  ...
  const [correctingPoints, setCorrectingPoints] = useState(false);
  const pointUpdateBusy = useRef(false);
  ...
  const initialPlaybackPosition = useRef(0);
  ...
  function applyAttempt(a: VideoAttempt, initial = false, playbackPosition = a.position) {
    ...
    if (initial) {
      ...
      setDrafts(next); draftRef.current = next; setPosition(playbackPosition); highWater.current = a.position; initialPlaybackPosition.current = playbackPosition;
    } else {
  ...
  useEffect(() => {
    ...
    const updateSize = () => {
      ...
      const controls = [".vq-round-nav", ".vq-jump-controls"].reduce((height, selector) => height + (player.querySelector<HTMLElement>(selector)?.offsetHeight || 0), 20);
  ...
  const togglePlayback = useCallback(() => {
    ...
    else { playIntent.current = false; v.pause(); if (!review) void sync(v.currentTime).catch(() => {}); }
  }, [review, sync]);
  useEffect(() => {
  ...
  async function openHistory(h: VideoHistory) {
    if (debounce.current) clearTimeout(debounce.current);
    playIntent.current = false; video.current?.pause(); setError("");
    try {
      const data = await request({ action: guest ? "open" : "review", attemptId: h.id });
      const playback = historyPlayback(data.attempt, guest);
      setReview(playback.review);
      if (data.episode) setCatalogue(old => old.map(e => e.id === h.quizId ? data.episode : e));
      applyAttempt(data.attempt, true, playback.position); setSelectedRound(null);
  ...
  function progress() {
    const v = video.current; if (!v || !episode) return;
    if (review) { setPosition(v.currentTime); return; }
    if (boundaryBusy.current || v.seeking) return;
  ...
  async function submit(questionId: string) {
    const v = video.current; if (!v || review || boundaryBusy.current) return;
    const at = v.currentTime, resume = playIntent.current, expectedId = attemptRef.current?.id;
    boundaryBusy.current = true; setSubmittingQuestion(questionId); v.pause();
    if (debounce.current) clearTimeout(debounce.current);
    try { await sync(at, questionId); if (resume && playIntent.current && attemptRef.current?.id === expectedId) await v.play(); } catch { /* keep the player paused for retry */ } finally { boundaryBusy.current = false; setSubmittingQuestion(null); }
  }
  async function jump(at: number) {
    ...
    if (review) {
      v.currentTime = Math.max(0, Math.min(at, episode?.duration ?? at));
      setPosition(v.currentTime); setSelectedRound(null); setError("");
      return;
    }
    if (!review && at < Math.max(highWater.current, a.position, v.currentTime) - .05) return;
    if (debounce.current) clearTimeout(debounce.current);
  ...
  function changePoints(questionId: string, value: number) {
    const a = attemptRef.current;
    if (!a || guest || pointUpdateBusy.current || boundaryBusy.current || dialogSubmitting.current) return;
    pointUpdateBusy.current = true; setCorrectingPoints(true);
    const pending = queue.current.catch(() => {}).then(async () => {
      if (attemptRef.current?.id !== a.id) return;
      const data = await request({ action: "correct", attemptId: a.id, questionId, points: value, reasonCode: "accepted-answer" });
      if (attemptRef.current?.id === a.id) applyAttempt(data.attempt);
    });
    queue.current = pending.catch(() => {});
    void pending.catch(e => { if (attemptRef.current?.id === a.id) setError((e as Error).message); }).finally(() => { pointUpdateBusy.current = false; setCorrectingPoints(false); });
  }
  ...
  if (!attempt || !episode || !round) return <section className="vq-library">
    {alerts}<div className="section-heading"><h1>Видеоквизы</h1></div>
    ...
    <section className="vq-history"><h2>Прохождения {guest ? "и результаты" : "всех игроков"}</h2>{history.length ? history.map(h => <button className="vq-history-row" key={h.id} onClick={() => void openHistory(h)}><span><b>{h.name}</b><small>{new Date(h.createdAt).toLocaleString("ru-RU")} · {h.completed ? "Завершён" : `Остановлен на ${time(h.position)}`} · {!guest || h.completed ? "Смотреть с начала" : "Продолжить игру"}</small></span><strong>{points(h.score)} / {points(h.maxScore)}</strong></button>) : <p>Здесь появятся сохранённые результаты и незавершённые игры.</p>}</section>
  ...
  const clock = activeChanceQuestion && chanceClock(activeChanceQuestion, position);
  ...
  return <section className="vq-game" ref={gameRef}>
    ...
    <nav className="vq-round-nav" aria-label="Бланки раундов">{episode.rounds.map(r => <button key={r.number} title={`Раунд ${r.number} · ${r.title}`} aria-label={`Раунд ${r.number} · ${r.title}`} aria-current={r.number === round.number ? "step" : undefined} className={r.number === round.number ? "active" : ""} disabled={jumping || !review && r.start < Math.max(position, attempt.position, highWater.current) - .05} onClick={() => void jump(r.start)}><span>{r.number}</span>{r.title}{attempt.answers[r.questions[0].id]?.points !== undefined && <Check size={13} />}</button>)}</nav>
    <div className="vq-player"><video key={attempt.id} ref={video} src={media} preload="metadata" playsInline controls={review} onLoadedMetadata={() => { if (video.current) video.current.currentTime = initialPlaybackPosition.current; }} onTimeUpdate={progress} onPlay={() => setPlaying(true)} onPause={() => setPlaying(false)} onSeeking={() => { const v = video.current; if (v && !review && Math.abs(v.currentTime - highWater.current) > .3) v.currentTime = highWater.current; }} onRateChange={() => { if (video.current && !review) video.current.playbackRate = 1; }} onEnded={() => { playIntent.current = false; setPlaying(false); setPosition(episode.duration); if (!review) void sync(episode.duration).catch(() => {}); }} onError={() => setError("Видео недоступно. Проверьте загрузку файла на сервер и соединение.")} aria-label={episode.title} />
      {!review && <div className="vq-player-controls"><button className="vq-play" disabled={submittingQuestion !== null || jumping} onClick={togglePlayback} aria-label={playing ? "Пауза · F2" : "Воспроизвести · F2"}>{playing ? <Pause /> : <Play />}</button><span>{time(position)} <small>/ {time(episode.duration)}</small></span><kbd>F2</kbd></div>}
    ...
    <div className="vq-sheet-heading"><div><span className="vq-kicker">РАУНД {round.number}</span><h2>{round.title}</h2></div><span className="vq-pill vq-deadline">{round.number === 7 ? clock ? <>{clock.phase === "music" ? "Музыка" : "На отправку"} · {time(clock.seconds)} · {points(clock.points)} б.</> : position < round.questions[0].start ? "Скоро первый шанс" : position < round.close ? "Ответ" : <><LockKeyhole size={15} /> Приём закрыт</> : position >= round.close ? <><LockKeyhole size={15} /> Приём закрыт</> : `До сдачи ${time(Math.max(0, Math.ceil(round.close - position)))}`}</span></div>
    ...
    <div className="vq-answer-sheet">{round.questions.map(q => {
      ...
      return <article id={`video-question-${q.id}`} key={q.id} className={`vq-answer-row ${active ? "is-active" : ""} ${revealed ? "is-revealed" : ""}`}><span className="vq-question-number">{String(q.number).padStart(2, "0")}</span><div className="vq-answer-content"><div className="vq-fields">{q.fields.map(field => <label key={field.key}><input autoComplete="off" spellCheck={false} maxLength={160} aria-label={`Вопрос ${q.number}: ${field.label}`} value={drafts[q.id]?.[field.key] || ""} disabled={locked || !!q.chances.length && !active} readOnly={submittingQuestion === q.id} onFocus={e => { const input = e.currentTarget; requestAnimationFrame(() => keepFieldVisible(input)); }} onChange={e => change(q.id, field.key, e.target.value)} placeholder={field.label} /></label>)}</div>
        {q.chances.length > 0 && !revealed && <div className="vq-chance-line"><span>{answer.submitted ? `Отправлено на ${time(answer.submittedAt || 0)}` : locked ? "Ответ не отправлен" : currentChance ? `Сейчас можно получить ${points(currentChance.points)} балл${currentChance.points === 1 ? "" : "а"}` : "Ожидает своего фрагмента"}</span>{!locked && <button className="vq-primary" disabled={!active || submittingQuestion !== null || !drafts[q.id]?.artist.trim()} aria-busy={submittingQuestion === q.id} onClick={() => void submit(q.id)}><Send size={14} /> Отправить</button>}</div>}
        {revealed && <div className="vq-reveal"><strong>{q.fields.map(f => answer.correct?.[f.key]).filter(Boolean).join(" — ")}</strong>{answer.coverArtist && answer.coverArtist !== "Кавер-версия" && <small>Кавер: {answer.coverArtist}</small>}{!!answer.links?.length && <span className="vq-source-links">{answer.links.map(link => <a key={link.url} href={link.url} target="_blank" rel="noopener noreferrer" onClick={() => { playIntent.current = false; video.current?.pause(); }}>{link.label} ↗</a>)}</span>}{!guest && <div className="vq-row-tools"><select aria-label={`Исправить баллы за вопрос ${q.number}`} disabled={correctingPoints || jumping} value={answer.points} onChange={e => changePoints(q.id, Number(e.target.value))}>{Array.from({ length: questionMaximum(q) * 2 + 1 }, (_, i) => i / 2).map(n => <option value={n} key={n}>{points(n)} балла</option>)}</select><button disabled={correctingPoints || jumping} onClick={() => { openDialog({ kind: "correct", questionId: q.id, annulled: !answer.annulled }); setComment(""); }}>{answer.annulled ? "Вернуть вопрос" : "Аннулировать"}</button></div>}</div>}
