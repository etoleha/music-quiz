...
export default function TrackReferenceCard({ info, track }: { info?: ArtistInfo; track: Track }) {
  ...
  const hasDetails = !!(ready?.members?.length || ready?.credits?.length || facts.length || sources.length || ready?.artistUrl || ready?.soundtrack || ready?.originalRecording || ready?.difficulty || ready?.image?.attribution || ready?.image?.license);
    ...
    <div className="track-reference__body">
      ...
      {hasDetails && <details className="track-reference__details">
        ...
        <div className="track-reference__expanded">
          ...
          {!!ready?.credits?.length && <section><h4>Авторы</h4><dl className="track-reference__credits">{ready.credits.map((credit, index) => <div key={`${credit.role}-${index}`}><dt>{credit.role}</dt><dd>{credit.names.join(", ")}</dd></div>)}</dl></section>}
          ...
          {ready?.difficulty && <section><h4>Сложность: {difficultyName[ready.difficulty.band].toLocaleLowerCase("ru-RU")}</h4><p>{ready.difficulty.explanation}</p></section>}
